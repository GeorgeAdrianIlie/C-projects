...
namespace BinaryOperations
{
    public class Program
    {
        ...
        private static void ExecuteFactorialOperation(int operation)
        {
        ...
        public static byte[] BinaryFactorial(byte[] firstBinaryNumber)
        {
            if (GetBinaryNumberRealLength(firstBinaryNumber) == 1)
            {
                return firstBinaryNumber;
            }

            byte[] subtrahend = { 1 };
            byte[] firstBinaryNumberMinusOne = BinarySubtract(firstBinaryNumber, subtrahend);
            byte[] result = BinaryMultiply(firstBinaryNumber, firstBinaryNumberMinusOne);
            while (GetBinaryNumberRealLength(firstBinaryNumberMinusOne) != 1)
            {
                firstBinaryNumberMinusOne = BinarySubtract(firstBinaryNumberMinusOne, subtrahend);
                result = BinaryMultiply(result, firstBinaryNumberMinusOne);
            }
        ...
        public static string BinaryNumberToString(byte[] binaryNumber)
        {
            string result = "";
